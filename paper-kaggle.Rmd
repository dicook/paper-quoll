...
MAST90083 used \url{https://inclass.kaggle.com/c}. ETC2420/5242 used \url{https://inclass.kaggle.com/c/vitticeps}. ETC3250 used \url{}.
...
On average the students who participated in the kaggle challenge scored `r dif` higher than those that didn't, a median of `r mean(scores$perf[scores$class=="Yes"])` in comparison to `r mean(scores$perf[scores$class=="No"])`. Using a permutation test, this corresponds to a significant difference in medians, with $p$-value of `r length(pdif[pdif >= dif])/1000`. A conventional two-sample $t$-test, of mean difference, yields similar results with a $p$-value of `r tt$p.value`.

...
scores_5242 <- scores_5242 %>% mutate(perf = ((`7b (8)`+ `7c (3)`)/11)/(`Exam Tot`/100))
scores_2420 <- scores_2420 %>% mutate(perf = ((`7b (8)`+ `7c (3)`)/11)/(`Exam Tot`/100))
...
#div <- function(x) x/poss_marks
#scores_pct[,2:27] <- apply(scores_pct[,2:27], 1, div)
scores_pct_long <- scores_pct %>% gather(q, mark, -group, -id)
ggplot(scores_pct_long, aes(x=q, y=mark, colour=group)) + geom_boxplot()
...
ggplot(ms, aes(x=q, y=m, colour=group)) + #geom_point(position="dodge") +

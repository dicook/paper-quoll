...
\end{tabular}
\caption{Computational statistics and data mining: Basics summary statistics of the exam score (out of 100) and the second assignment (out of 10) for the two competition groups.}
\end{center}
...
\begin{tabular}{l l r r r r r r r}\hline
Score & Competition group & N & Mean (SD)   & Min  &  Q1 & Median  &  Q3  & Max \\\hline
Exam   & ST-PG      & 34  & 61.8 (12.2)  &  35 & 57.2  &   61  & 72.2  & 81.5 \\
       & ST-UG         & 145 & 63.0 (15.4)  &   0 & 56.0  &   64  & 73.0   & 90.0\\\hline
\end{tabular}
\caption{Statistical thinking: Basics summary statistics of the exam score (out of 100) for the two groups.}
\end{center}
\label{tab:Melb_Group_Compare}
\end{table}

Figure \ref{fig:Monash_quiz} presents the means and the standard deviations of the quizzes taken by the undergraduate (ST-UG) and postgraduate (ST-PG) students. 
...
```{r Monash_quiz, fig.cap = "\\label{fig:Monash_quiz} Means and error bars of undergraduate (ST-UG) and postgraduate (ST-PG) students quizes during the semester.", fig.height=4.5, fig.width=6}
library(tidyverse)
library(gridExtra)
...
quiz1 <- quiz %>%   mutate(unit=replace(unit, unit=='2420','ST-UG')) %>%
  mutate(unit=replace(unit, unit=='5242','ST-PG'))
...
```{r Monash_quiz_mean_diff, fig.cap = "\\label{fig:Monash_quiz_mean_diff} Mean difference of undergraduate (ST-UG) and postgraduate (ST-PG) students quizes scores and CI.", fig.height=4.5, fig.width=6}

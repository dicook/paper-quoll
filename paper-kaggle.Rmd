...
```{r Monash_quiz, fig.cap = "\\label{fig:Monash_quiz}Beeswarm plots of undergraduate (ST-UG) and postgraduate (ST-PG) students quizzes during the semester. The scores are very similar, although undergraduates have a longer lower tail.", fig.height=6, fig.width=6, eval=FALSE}
library(tidyverse)
library(gridExtra)
library(ggbeeswarm)
library(forcats)
...
ggplot(quiz, aes(x=score^4, colour=unit, fill=unit)) +
  geom_density(alpha=0.5) +
  facet_grid(unit~quiz, scales="free") + 
  xlab("") + ylab("Score (%)") +
  theme(legend.position="none")

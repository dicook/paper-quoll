...
Kaggle [@kaggle] is well-known for the data competitions, some richly funded. It provides a platform for predictive modelling and analytics competitions where participants compete to produce the best predictive model for a given data set. One of the well-known competitions is the ``Titanic: Machine Learning from Disaster'' in which, mainly the beginners, can practice their skills to predict survival on the Titanic and get familiar with the Kaggle platform and some of the machine learning techniques.      


*Di any``famous'' data competition examples ???*

The competitors participating in these competitions have different motivations, but it is mainly fame and financial reward for the winner. 
Occasionally, inventors of new methods and algorithms use Kaggle competitions as a platform to prove the superiority of their methods. A famous example are Geoffrey Hinton and George Dahl with deep neural networks [@HintonDahi12] and Tianqi Chen and Carlos Guestrin with XGBoost [@ChenXGBoost].

Individuals and organizations that contribute the data and open it for a competition gain expertise from many professionals at once. This increases the chance for an improved and innovative solution to their problem.
...
In the past few years, educational community started to collect positive evidence on including a competition in the classroom (see for example @Calnon12, @Canada15 and @VanNuland15).
The most comparable to our experiment is the educational experiment conducted by @VanNuland15. 
The authors found significant improvement in student's exam marks due to taking part in the competition. The experiment (crossover design) was conducted in a similar class size ($n=67$). However, these were undergraduate students that took the human anatomy subject at Western University.

The competition is an example of active learning. A large body of literature has been published on the latter topic. @Prince04
surveyed the literature and found that all forms of active learning have positive effect on the learning experience and student's achievements. The magnitude of the effect of different approaches, though, varies. What's more, @Freeman14 examined 158 studies published in about 50 STEM educational journals. The authors found that student's exam marks increased by almost half SD under active learning. Moreover, students in classes with traditional lecturing were 1.5 times more likely to fail than their peers in classes with active learning.

A competition, like any other assessment method has its advantages and disadvantages. It brings the 'game' feeling, increases the interest level among students and motivates for higher performance (@Shindler09, p 105). But, it may have negative influence if set wrongly. Among the negative influences are potential stress and increased anxiety (of low ranking, fear to fail or technology barrier). In addition, students may invest disproportional time and effort into competition.      

Despite some received criticism, a properly set competition can benefit the students greatly. The competition should be relatively short in duration as it consumes a lot of students' energy. Students should be clear about the rules and the goal. They should be properly rewarded and most important, feel that they have reasonable chance to win or achieve high mark [@Shindler09]. 

@Shelley08 raised the need for more quantitative and statistical analysis of evidence in science education. In this paper we will contribute to this call by offering statistical analysis of the experiment's results. 
...
## Working with the Kaggle InClass platform


Creating a new competition is surprisingly easy. 
Information on setting up a Kaggle InClass challenge can be found at https://www.kaggle.com/about/inclass/overview. 
To ease the setting process, we created a short video illustrating the steps to establish a competition....

Finding data for a competition may take some time. It is not recommended to take freely available data sets from the Internet. A clever student will always try to check if the data is on the web. If it is, she will use the answers from the data set instead of predicting the answers with a self-developed model. As a statistician, and an instructor in a subject, you or one of your colleagues usually have some interesting data set to use. In such a case, the data set and particularly the target values will be unfamiliar to the students.

Once you open the competition, it runs without any intervention from the instructor. The instructor can monitor students progress: the number of submissions, student scores and even the uploaded data at any time. When the competition ends the "Leaderboard" page provides a list of students ordered by the final score. It also provides all the scores from all past submissions (under "Raw Data" on "Public Leaderboard"). 

From our experience, we recommend limiting the number of daily submissions to one. It encourages students to think about more efficient improvement of their model before the next submissions.  We also believe that it is better to start the competition in the second half of the semester and finish it two to one week before the end of the semester. It allows to cover many different statistical methods that students may apply during the competition. Finishing some time before the end of the semester reduces students anxiety at the end of the semester and the approaching exams.  
